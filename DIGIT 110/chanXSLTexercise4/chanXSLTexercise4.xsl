...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="chapter" mode="toc">
        <tr>
            <td><a href="#C{count(preceding-sibling::chapter) + 1}"></a><xsl:apply-templates select="heading"/></td>
            <td><xsl:apply-templates select="string-join(distinct-values(descendant::location),', ')"/><xsl:if test="location[@where]"><xsl:apply-templates/></xsl:if></td>
            <td><xsl:apply-templates select="string-join(distinct-values(descendant::tech[@type]), ', ')"/></td>   
        </tr>
    </xsl:template>
